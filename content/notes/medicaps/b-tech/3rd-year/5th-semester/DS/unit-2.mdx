...
</math>is the probability that (X) lies between (a) and (b).
...
The **Cumulative Distribution Function (CDF)** represents the probability that a random variable (X) is less than or equal to a particular value (x). It is the integral of the PDF from -infinity to (x).
