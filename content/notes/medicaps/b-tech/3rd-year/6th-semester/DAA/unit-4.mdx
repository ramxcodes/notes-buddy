...
description: Dynamic programming problems and its complexity analysis 0/1 Knapsack, Multistage graph, Bellman Ford Algorithm, Reliability design, Floyd-Warshall algorithm, Matrix Chain Multiplication, Longest Common subsequence.
date: 2025-01-19
...
3. **Recurrence Relation**:
   ...
   \begin{cases} 
   dp[i-1][w] & if w_i > w \\
   \max(dp[i-1][w], v_i + dp[i-1][w - w_i]) & if w_i \leq w
   \end{cases}
   $$
   Here:
...
1. **Recursive Structure**:
   ...
   \max(LCS(i-1, j), LCS(i, j-1)) & if X[i] \neq Y[j]
   \end{cases}
   $$
